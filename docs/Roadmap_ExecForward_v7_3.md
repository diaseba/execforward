...
**versión 7.3 · actualizado 26 may 2026**
...
| Estado actual | S3a en curso · T3.5 ✅ · T3.5b + T3.6 en curso (Cowork) |
...
| 9 | **Onboarding:** pantalla inicial "¿cuál fue tu score?" → personaliza orden del sidebar + mensaje motivacional por banda de score. Pre-fill automatico si el usuario hizo el quiz antes de comprar. |
...
| 11 | **Quiz M0 landing:** HTML estático · score → texto copiable → CTA personalizado al SKU correcto. Score, banda y módulo débil se persisten en localStorage. |
...
| 17 | **Cierre (pantalla final M6):** texto aprobado — aparece solo cuando TODOS los módulos accesibles del SKU están completos al 100%. |
...
| 23 | **Aprobación LS:** LS requiere ver web y producto antes de aprobar. Cowork construye todo sin LS. Post-aprobación (≤�8h): find-and-replace de **5 placeholders** `[LS_URL_*]` en `app-utils.js` < 30 min. Los 5 placeholders: `[LS_URL_START]` · `[LS_URL_CORE]` · `[LS_URL_BUNDLE]` · `[LS_URL_BUNDLE_UPGRADE_START]` · `[LS_URL_BUNDLE_UPGRADE_CORE]`. |
...
| 25 | **Gate S3 dividido:** S3-A = producto funcional probado por círculo del operador bría /test (sin LS). S3-B = LS aprobado → URLs insertadas → compra real end-to-end. S3-A y S3-B corren en paralelo. |
...
| 27 | **Flujos de transición y upsell:** al completar cada módulo aparece pantalla de transición → siguiente módulo. Al completar el último módulo del SKU → pantalla de upsell (solo /start y /core). Al completar M6 con módulos pendientes → pantalla "Casi llegas" con cards dinámicas. Precios de upgrade: $60 (Start→Bundle) y $30 (Core→Bundle). |
...
Post-purchase redirect → execforward.com/programa/[SKU]
 └── Pantalla onboarding "¿cuál fue tu score?" (pre-fill desde localStorage si hizo quiz)
      └── Sidebar personalizado por banda de score
           └── M1 → M2 → M3 → M4 → M5 → M6
                ...
                · Pantalla de transición al completar cada módulo
                · Upsell al completar último módulo del SKU (/start y /core)
                · Pantalla "Casi llegas" si M6 completo pero hay pendientes (/bundle)
                · Pantalla de cierre final cuando TODOS los módulos completos

ENTORNO DE PRUEBA
──────────────────────────────
execforward.com/programa/test ✅ OPERATIVO
...
──────────────────────────────
5 placeholders: [LS_URL_START] / [LS_URL_CORE] / [LS_URL_BUNDLE]
                [LS_URL_BUNDLE_UPGRADE_START] / [LS_URL_BUNDLE_UPGRADE_CORE]
 └── Viven en app-utils.js
 └── Find-and-replace post-aprobación LS · < 30 min
...
| Tarea | Ejecutor | Estado | S1 | S2 | S3a | S3b | S4 | S5 | S6 |
|---|---|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **T1.1** Cerrar decisiones fundacionales | Tú · Claude | ✅ | ✅ | | | | | | |
| **T1.2** Crear tienda LS + conectar payout | Cónyuge | ⏳ | ⏳ | | | | | | |
| **T1.3** Dominio + logo | Cónyuge | ✅ | ✅ | | | | | | |
| **T1.4** Contenido M0–M6 completo (ES) | Claude · Tú | ✅ | ✅ | | | | | | |
| **T2.1** Spec técnica web app para Cowork | Claude · Tú | ✅ | | ✅ | | | | | |
| **T2.2** Spec quiz M0 landing para Cowork | Claude · Tú | ✅ | | ✅ | | | | | |
| **T2.3** Copy landing ES master | Claude · Tú | ✅ | | ✅ | | | | | |
| **T2.4** Diseño email score (resultado quiz) | Claude · Tú | ✅ | | ✅ | | | | | |
| **T2.5** Landing page live | Cowork | ✅ | | ✅ | | | | | |
| **T2.6** Quiz M0 funcional en /quiz | Cowork | ✅ | | ✅ | | | | | |
| **┊ Gate S2** Specs aprobadas → Cowork arrancó | Tú | ✅ | | ┊ | | | | | |
| **T3.1** Cowork — estructura web app shell (4 versiones) | Cowork | ✅ | | ✅ | | | | | |
| **T3.2** Cowork — integrar contenido M0–M6 en web app | Cowork | ✅ | | ✅ | | | | | |
| **T3.4** Cowork — mindmaps SVG (uno por módulo M1–M6) | Cowork | ✅ | | ✅ | | | | | |
| **T3.7** Logo v2 + brandbook | Tú · Claude | ✅ | | | ✅ | | | | |
| **T3.5** Cowork — pantallas onboarding y cierre | Cowork | ✅ | | | ✅ | | | | |
| **T3.5b** Cowork — flujos transición/upsell/cierre (corrección) | Cowork | ⏳ | | | ⏳ | | | | |
| **T3.6** Cowork — plantillas email confirmación LS (3 SKUs) | Cowork | ⏳ | | | ⏳ | | | | |
| **T3.3** Producir audio + video NotebookLM por módulo | Tú | ⬜ | | | ⬜ | | | | |
| **T3.8** Revisión completa web app (checklist funcional) | Tú · Claude | ⬜ | | | ⬜ | | | | |
| **T3.9** Pase de brandbook sobre web app completa | Cowork | ⬜ | | | ⬜ | | | | |
| **T3.10** Cowork — variantes PT/EN/FR (landing + web app) | Cowork | ⬜ | | | | ⬜ | | | |
| **T3.11** Cargar SKUs en LS (Premium deshabilitado) | Cónyuge | ⬜ | | | | ⬜ | | | |
| **T3.12** Someter producto a aprobación LS | Cónyuge · Tú | ⬜ | | | ⬜ | | | | |
| **T3.13** Aprobación LS recibida (≤48h) | LS | ⬜ | | | | ⬜ | | | |
| **T3.14** Find-and-replace `[LS_URL_*]` en app-utils.js (5) | Tú · Cónyuge | ⬜ | | | | ⬜ | | | |
| **T3.15** Configurar afiliados en LS (oculto) | Cónyuge · Claude | ⬜ | | | | ⬜ | | | |
| **T3.16** Post LinkedIn + mensajes afiliados | Claude · Tú | ⬜ | | | | ⬜ | | | |
| **┊ Gate S3-A** /test con M1–M6 completos · probado por círculo | Tú · círculo | | | | | ┊ | | | |
| **┊ Gate S3-B** LS aprobado + compra real end-to-end | Tú · Cónyuge | | | | | ┊ | | | |
| **T4.1 ▲** Disparo único LinkedIn (10.429) | Tú | ⬜ | | | | | ⬜ | | |
| **T4.2** Monitoreo pasivo métricas (≤30 min/día) | Cónyuge | ⬜ | | | | | ⬜ | ⬜ | ⬜ |
| **┊ Hito S4** Diagnóstico tracción post-disparo | Tú · Claude | | | | | | ┊ | | |
| **T5.1 ▲** Outreach afiliados (una vez) | Tú | ⬜ | | | | | | ⬜ | |
| **T5.2** Activar signup afiliados + auto-aprobación | Cónyuge | ⬜ | | | | | | ⬜ | |
| **T5.3** Pack creatives para afiliados | Cowork | ⬜ | | | | | | ⬜ | |
| **┊ Hito S6** Veredicto tracción + gatillo Fase 2 | Tú · Claude | | | | | | | | ┊ |

**Notas:**
- T3.1, T3.2, T3.4 completados (Pasos 3–5). Shell + contenido M1–M6 + mindmaps SVG entregados.
- T3.7 completado: logo v2 + brandbook cerrados. 5 SVGs en /logo/. Brandbook HTML en Drive.
- T3.5 completado (Paso 6): onboarding + cierre construidos. 4 gaps resueltos por Cowork.
- T3.5b + T3.6 en curso (Pasos 7b + 7): flujos transición/upsell/cierre + emails LS — tarea entregada.
- T3.9 pase de brandbook: un solo pase sobre todo el producto después de T3.8.
- T3.12 someter a LS: cuando /test tenga M1–M6 completos + brandbook aplicado.
- T3.14: ahora son 5 placeholders (3 originales + 2 de upgrade con descuento).
- Gate S3-A y S3-B corren en paralelo. S3-B no puede cerrarse sin S3-A.
- ▲ = acción única permitida por las reglas del proyecto.
...
| **S3a** | Logo v2 ✅ · Brandbook ✅ · T3.5 ✅ · T3.5b + T3.6 en curso · M0 �M6 en /test · audio/video NbL� producido | /test navegable completo con flujos correctos · brandbook entregado a Cowork | Cowork · Tú · Claude |
...
| Venta upgrade $60 (Start→Bundle) | ≈ $54 | Idem |
| Venta upgrade $30 (Core→Bundle) | ≈ $26 | Idem |
...
|---|---|---|
| `Roadmap_ExecForward_v7_3.md` | Este archivo | v7.3 |
| `Brandbook_ExecForward_v1.md` | Estrategia de marca + sistema visual completo | v1.0 |
| `Spec_Tecnica_ExecForward_Cowork_v1_1.md` | Spec técnica completa para Cowork | v1.1 |
| `Handoff_Cowork_ExecForward_v1.md` | Handoff completo con copy y orden de ejecución | v1.0 |
| `Tarea_Cowork_Pasos_7_y_7b.md` | Tarea activa Cowork — emails LS + flujos transición/upsell/cierre | v1.0 |
...
| `MD_Red_de_Contactos_y_Networking.md` | Módulo 4 | — |
...
| `logo/EF_logo_B_horizontal.svg` | Logo horizontal — anavbar, email, docs | v1.0 |
...
> Roadmap v7.3 — cambios vs v7.2: T3.5 ✅. T3.5b agregado (corrección flujos transición/upsell/cierre). Decisión 27 cerrada (flujos y precios de upgrade). Decisión 23 actualizada (5 placeholders). Economía unitaria con upgrades. T3.14 actualizado a 5 placeholders.

| # | Tarea | Quién | Cuándo |
|---:|---|---|---|
| 1 | ⏳ T3.5b — Flujos transición/upsell/cierre (corrección) | Cowork | Ahora |
| 2 | ⏳ T3.6 — Plantillas email confirmación LS (3 SKUs) | Cowork | Ahora (paralelo) |
| 3 | ⬜ T3.3 — Producir audio + video NbL� por módulo | Tú | S3a (paralelo) |
| 4 | ⬜ T3.8 — Revisión funcional completa de /test | Tú · Claude | Post T3.5b + T3.6 |
...
| 7 | ⬜ T3.14 — Find-and-replace 5 `[LS_URL_*]` en app-utils.js | Tú | Post-aprobación LS |
| 8 | ⬜ Gate S3-A — Prueba de círculo vi� /programa/test | Tú · círculo | Cierre S3a |

---

*ExecForward · Roadmap v7.3 · 26 may 2026*